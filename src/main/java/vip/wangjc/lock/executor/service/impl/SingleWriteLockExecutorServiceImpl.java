 ...
 * 单节点的写锁执行器
 * @author wangjc
 * @title: SingleWriteLockExecutorServiceImpl
 * @projectName wangjc-vip
 * @date 2020/12/12 - 16:45
 */
public class SingleWriteLockExecutorServiceImpl implements ILockExecutorService {
    ...
    public boolean acquire(String key, String value, Long timeout, Long expire) {
        try {
             ...
             */
            ReentrantReadWriteLock.WriteLock writeLock = LockSinglePool.getWriteLock(key);
    ...
    public boolean release(LockEntity lockEntity) {
        if(lockEntity != null){
            LockSinglePool.getLock(lockEntity.getKey(),ReentrantReadWriteLock.WriteLock.class).unlock();
            return true;
        }
        return false;
    }
